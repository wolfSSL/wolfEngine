...
of them on but this list will provide full functionality.
...
It is enough to compile the wolfEngine solution to generate the DLL file. The
`test` project in the wolfEngine solution compiles an executable file that is
dynamically linked to the wolfEngine and OpenSSL libraries. The `test` 
solution copies the OpenSSL DLL files into the output directory. If you want
to skip this step and use system supplied versions of OpenSSL, delete 
the command under:
```
