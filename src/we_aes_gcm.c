                           ...
                           const unsigned char *iv, int enc)
{
    ...
    if ((key == NULL) && (iv == NULL)) {
        XMEMSET(aes, 0, sizeof(we_AesGcm));
        wc_AesInit(&aes->aes, NULL, INVALID_DEVID);
    }
                             ...
                             const unsigned char *in, size_t len)
{
    ...
    /* Length may be zero for cases with AAD data only (GMAC) */
        ...
        if (aes->enc) {
            ...
            if (ret == 1) {
                if (len > 0) {
                    rc = wc_AesGcmEncryptUpdate(&aes->aes, out, in, (word32)len,
                        NULL, 0);
                    if (rc != 0) {
                        WOLFENGINE_ERROR_FUNC(WE_LOG_CIPHER,
                                          "wc_AesGcmEncryptUpdate", rc);
            ...
            if (ret == 1) {
                ...
                rc = wc_AesGcmEncryptFinal(&aes->aes, aes->tag, aes->tagLen);
                if (rc != 0) {
                    WOLFENGINE_ERROR_FUNC(WE_LOG_CIPHER,
                                          "wc_AesGcmEncryptFinal", rc);
                    ret = 0;
                }
            }
            ...
            if (ret == 1) {
                if (len > 0) {
                    rc = wc_AesGcmDecryptUpdate(&aes->aes, out, in, (word32)len,
                        NULL, 0);
                    if (rc != 0) {
                        WOLFENGINE_ERROR_FUNC(WE_LOG_CIPHER,
                                          "wc_AesGcmDecryptUpdate", rc);
            ...
            if (ret == 1) {
                rc = wc_AesGcmDecryptFinal(&aes->aes, aes->tag, aes->tagLen);
                if (rc != 0) {
                    WOLFENGINE_ERROR_FUNC(WE_LOG_CIPHER,
                                          "wc_AesGcmDecryptFinal", rc);
                    ret = 0;
                }
            }
