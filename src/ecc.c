...
static int we_ec_set_private(ecc_key *key, int curveId, const EC_KEY *ecKey)
{
    ...
    if (privLen <= 0) {
        WOLFENGINE_ERROR_MSG("EC_KEY_priv2buf failed");
        ret = 0;
    }
    /* Import private key. */
    if (ret == 1) {
...
static int we_ec_p256_init(EVP_PKEY_CTX *ctx)
{
    ...
    if (ret == 1) {
        ...
        if (ecc->group == NULL) {
            /* Failed - free allocated data. */
            WOLFENGINE_ERROR_MSG("EC_GROUP_new_by_curve_name failed");
            wc_ecc_free(&ecc->key);
            OPENSSL_free(ecc);
            ret = 0;
        }
...
static void we_ec_cleanup(EVP_PKEY_CTX *ctx)
{
    ...
    WOLFENGINE_LEAVE("we_ec_cleanup", 0);
}
...
static int we_ec_get_ec_key(EVP_PKEY_CTX *ctx, EC_KEY **ecKey, we_Ecc *ecc)
{
    ...
    if (ret == 1) {
        ...
        if (group == NULL) {
            WOLFENGINE_ERROR_MSG("EC_KEY_get0_group failed");
            ret = 0;
        }
    }
    if (ret == 1) {
...
static int we_ec_keygen(EVP_PKEY_CTX *ctx, EVP_PKEY *pkey)
{
    ...
    if (ret == 1) {
        ...
        if (ret == 0) {
            WOLFENGINE_ERROR_MSG("EVP_PKEY_assign_EC_KEY failed");
            EC_KEY_free(ecKey);
        }
    }
...
static int we_ec_ctrl(EVP_PKEY_CTX *ctx, int type, int num, void *ptr)
{
    ...
    ecc = (we_Ecc *)EVP_PKEY_CTX_get_data(ctx);
    if (ecc == NULL) {
        WOLFENGINE_ERROR_MSG("EVP_PKEY_CTX_get_data failed");
        ret = 0;
    }
    ...
    if (ret == 1) {
        switch (type) {
            ...
            /* Unsupported type. */
            default:
                WOLFENGINE_MSG("Unsupported control command type");
                ret = 0;
                break;
        }
    }
