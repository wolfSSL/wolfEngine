...
/**
 * Data required to complete an AES-CBC encrypt/decrypt operation.
 */
typedef struct we_AesCbc
{
 ...
 *
 * @param  ctx  [in/out]  EVP cipher context of operation.
 * @param  aes  [in]      Internal AES-CBC object.
 ...
 */
static int we_aes_cbc_encrypt(EVP_CIPHER_CTX *ctx, we_AesCbc* aes,
 ...
 * @return  Number of bytes put in out on success.
 */
static int we_aes_cbc_decrypt(EVP_CIPHER_CTX *ctx, we_AesCbc* aes,
    unsigned char *out, const unsigned char *in, size_t len)
{
    int ret = 1;
    int rc;
...
static int we_init_aescbc_meth(EVP_CIPHER *cipher)
{
    ...
    if (ret == 1) {
        ret = EVP_CIPHER_meth_set_impl_ctx_size(cipher, sizeof(we_AesCbc));
    }
...
int we_init_aescbc_meths()
{
    ...
    if (we_aes128_cbc_ciph == NULL) {
        WOLFENGINE_ERROR_MSG("EVP_CIPHER_meth_new - AES-128-CBC");
        ret = 0;
    }
    if (ret == 1) {
    ...
    if (ret == 1) {
        ...
        if (we_aes256_cbc_ciph == NULL) {
            WOLFENGINE_ERROR_MSG("EVP_CIPHER_meth_new - AES-256-CBC");
            ret = 0;
        }
    }
    if (ret == 1) {
